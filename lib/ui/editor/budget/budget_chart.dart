...
class BudgetChart extends StatelessWidget {
  ...
  LineChartData _getBudgetChartData(BuildContext context) {
    double maxAmount = data?.map((e) => e.amount.asDouble).lastOrNull ?? limit.asDouble;
    ...
    return LineChartData(
      ...
      lineBarsData: [
        if (data != null && data!.isNotEmpty) _getBudgetLineChartBarData(context, data!),
  ...
  LineChartBarData _getBudgetLineChartBarData(BuildContext context, Iterable<MoneyDateInfo> data) {
  ...
  LineChartBarData _budgetLine(BuildContext context, double amount, Iterable<MoneyDateInfo>? data) {
    List<DateTime>? timeline = data?.map((e) => e.date).toList();
    double start = 0;
    double end = timeline?.let((it) => it.lastOrNull?.difference(it.first).inDays.toDouble()) ?? 30;
    return LineChartBarData(
        isCurved: false,
