...
Future<void> _saveTransfer(
  ...
  try {
    final fromCurrency = await currenciesRepo.getOrSave(draft.fromCurrencyName!);
    final fromAccount = await accountsRepo.getOrSave(draft.fromAccountName!, fromCurrency);

    final toCurrency = await currenciesRepo.getOrSave(draft.toCurrencyName!);
    final toAccount = await accountsRepo.getOrSave(draft.toAccountName!, toCurrency);
