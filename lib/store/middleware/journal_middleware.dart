...
Middleware journalMiddleware(JournalRepo repo) => (PortmoneStore store, PortmoneAction action, NextDispatcher next) async {
...
void _refreshJournal(PortmoneStore store, JournalRepo repo, MainFilter filter) => Future(() async {
  ...
  final journal = groupBy(records, (r) => r.date.withoutTime)
    .entries
