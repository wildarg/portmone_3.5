...
Middleware budgetMiddlware(BudgetRepo repo, CurrenciesRepo currenciesRepo) => (PortmoneStore store, PortmoneAction action, NextDispatcher next) async {
  ...
  if (action is SaveBudgetAction) {
    ...
    final currency = await currenciesRepo.getOrSave(d.currencyName ?? 'USD');
