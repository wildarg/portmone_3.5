...
typedef Reducer =
    Future<void> Function(PortmoneStore store, PortmoneAction action);

typedef Middleware =
    Future<void> Function(
      PortmoneStore store,
      PortmoneAction action,
      NextDispatcher next,
    );
...
class PortmoneStore {
  ...
  Future<void> _reducer(PortmoneStore store, PortmoneAction action) async {
    return switch (action) {
      ...
      SetAccountBalanceReportAction() => accountBalanceState.sink.add(
        action.data,
      ),
  ...
  NextDispatcher _createDispatcher() {
    final dispatchers = <NextDispatcher>[
      (PortmoneAction action) => _reducer(this, action),
    ];
    for (var middleware in _middlewares.reversed) {
      final next = dispatchers.last;
      dispatchers.add(
        (PortmoneAction action) => middleware(this, action, next),
      );
    }
