...
class MoneyFormatTextEditingController extends TextEditingController {
  ...
  MoneyFormatTextEditingController({
    super.text,
    int? cents
  }) {
    _textWatcher = () {
  ...
  TextEditingValue applyFormat() {
    ...
    String leftFromCursor = (!cursorWasInFractionalPart? input.substring(0, cursorPosition) : "")
      .replaceAll(Separator.group, "");
    String intPart = parts[0].asDouble.formattedMoney;
    String fractPart = parts.length == 2? parts[1].limitFromStart(2)!
      .replaceAll(Separator.decimal, "")
      .replaceAll(Separator.group, "") 
      : "";

    String newText = "$intPart${parts.length == 2?Separator.decimal + fractPart : ''}";
