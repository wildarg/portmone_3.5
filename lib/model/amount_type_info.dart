...
class AmountTypeInfo {
  ...
  int get trendingSign {
    ...
    final hasData =
        chartData.length > 1 &&
        chartData
            .take(chartData.length - 1)
            .any((e) => e.amount.amountInCents != 0);
    if (!hasData) return 0;
    return chartData.last.amount.amountInCents.compareTo(
      chartData.toList()[chartData.length - 2].amount.amountInCents,
    );
  }
}
