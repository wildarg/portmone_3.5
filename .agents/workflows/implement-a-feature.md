...
1.  **CUnderstand Requirements**: Ask questions if something needs to be clarified.
2.  **Create Plan**: As a Senior Engineer, draft an `implementation_plan.md`. Do not just list files to change. Provide a high-level architectural rationale, discuss potential trade-offs, identify edge cases, and propose a robust testing strategy along with the architecture and UI updates.
